...
public class GetUsersTests extends BaseTest {
    ...
    @Test(groups = {"smoke", "regression"}, description = "GET paginated user list returns 200 and valid schema")
    ...
    public void testGetUserListSuccess() {
        Response response = client.queryParam("page", 2).get(UserEndpoints.USERS);
    ...
    @Test(groups = {"smoke", "regression"}, description = "GET single user by valid id returns 200")
    @Severity(SeverityLevel.CRITICAL)
    ...
    @Test(groups = {"regression", "negative"}, description = "GET user with non-existent id returns 404")
    @Severity(SeverityLevel.NORMAL)
    public void testGetSingleUserNotFound() {
        Response response = client.pathParam("id", 999999).get(UserEndpoints.USER_BY_ID);
    ...
    @Test(groups = {"regression"}, description = "GET user list pagination - total pages is consistent with total/per_page")
    @Severity(SeverityLevel.MINOR)
    public void testPaginationConsistency() {
        Response response = client.queryParam("page", 1).get(UserEndpoints.USERS);
        UserListResponse body = response.as(UserListResponse.class);

        int expectedTotalPages = (int) Math.ceil((double) body.getTotal() / body.getPerPage());
        assertThat(body.getTotalPages()).isEqualTo(expectedTotalPages);
        assertThat(body.getData()).hasSizeLessThanOrEqualTo(body.getPerPage());
    }

    @Test(groups = {"regression"}, description = "GET user list response time is within SLA")
    @Severity(SeverityLevel.MINOR)
