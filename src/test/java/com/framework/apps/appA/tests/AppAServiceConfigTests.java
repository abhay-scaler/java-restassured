 ...
 * ServiceConfig class automatically becomes scoped to App A simply by
 * virtue of this suite running under -Dapp=appA; ServiceConfig itself
...
public class AppAServiceConfigTests extends BaseTest {

    @Test(groups = {"regression"},
            description = "ServiceConfig(\"configtestonly\") resolves App A's own test-only " +
                    "services.configtestonly.base.url, not App B's")
    @Severity(SeverityLevel.NORMAL)
    @Description("Config-only assertion - ServiceConfig never makes a network call on its own.")
    public void testNamedServiceResolvesAppAsTestValue() {
        ServiceConfig configTestOnly = ServiceConfig.of("configtestonly");

        assertThat(configTestOnly.baseUrl()).isEqualTo("https://appa-configtestonly.test.example.com");
        assertThat(configTestOnly.baseUrl()).isNotEqualTo("https://appb-configtestonly.test.example.com");
    }
}
